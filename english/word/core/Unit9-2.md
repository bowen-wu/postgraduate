# Unit 9

- 感情色彩正向: 🙂
- 感情色彩负向: 😔
- vt. = transitive verb（及物动词）
    - 动词 + 宾语
    - 后面必须接宾语
    - **不**需要加介词
    - He **likes** apples
- vi. = intransitive verb（不及物动词）
    - 动作**不需要对象**就能完整表达意思
    - 动词 + 介词 + 宾语
    - He **sleeps** early.

## professor

- n. 教授
- profession n. 职业；专业
- professional adj. 职业的；专业的
- professionalize vt. 使专业化
- professionalism n. 专业水平
- amateur 
    - n. 业余爱好者
    - adj. 业余的
- expert n. 专家
- specialist
    - n. 专家； 
        - == expert
    - adj. 专业的
    - Yet only one of his books is now in print，and his <ins>vast body of(== oceans of 海量)</ins> writings on music is unknown <ins>save to(== except 除了)</ins> **specialists**.

## quality

- n. 质量；品质
- qualify 
    - v. 取得资格，有资格
    - vt. 使有资格
- qualified
    - adj. 合格的；有资格的
    - be qualified to do sth
- qualification n. 资格
- eligible
    - adj. 合格的；符合条件的
    - == qualified
    - Only If the jobless arrive at thejobcentre with a CV, register for online job search, and start looking for work will they be **eligible** for <ins>benefit(补助金；津贴)</ins>.

## apply

- vt. 使用；应用；涉及
- vi. 涉及；申请
- But the skills they learn **apply to(适用于；涉及)** any coding language.
- ...may lack oppotunities to **apply for(申请；请求)** research projects.
- applicable
    - adj. 可适用的
    - be applicable to 适用于
- applicant n. 申请人
- application n. 申请；应用；应用程序
- exert
    - vt. 施加（影响、压力）
    - These designers came to <ins>**exert** *enormous/profound* influence on(产生深远影响)</ins> the *course/development* of American architecture, but none more so than Mies.
- utilize vt. 利用

## balance

- vt. 平衡；均衡
- n. 平衡；余额
- check and balance 制约
- imbalance n. 不平衡
- counterbalance vt. 平衡；抵消

## seek

- v. 寻求；寻找
- vt. 请求
- == look for == pursue == search
- **Seek** first to understand then to be understood.
- seek out 找到；追寻到
- seek to 设法；争取
- jobseeker n. 求职者
- search
    - v. 搜寻；搜查；n. 搜寻；搜查；（在计算机上）搜索
    - search for 寻找；搜索
    - in search for 寻找；搜寻
- hunt
    - v. 搜寻；打猎
    - hunt for 搜寻；寻找
    - hunt down 对...穷追到底
    - hunter n. 猎人
    - headhunter n. 猎头
        - "The traditional rule was it's safer to stay where you are, but that's been fundamentally inverted" says one **headhunter**.

## detail

- vt. 详述
- n. 细节；详情
- in detail 详细地
- elaborate
    - v. 详细说明，详尽阐述；精心制作
    - adj. 详尽的，复杂的； 
    - == detailed
    - elaborate layout 精心的布局
- concrete
    - adj. 确实的；具体的；实在的
- simplify
    - vt. 简化
    - simple == easy
- lengthy adj. 漫长的；长时间的
- brief // TODO 
- briefly adj. 简单的；短暂的； n. 简要，简报
- roughly adv. 大约

## moral

- adj. 道德的
- n. 寓意
- ethics 
    - n. 道德准则；伦理学；伦理
    - ethical adj. 道德的；伦理的；合乎情理的
- virtue 🙂
    - n. 美德；优点，优势
    - But completeness is no longer a **virtue** in the newspaper business.

## necessary

- adj. 必需的；必然的
- n. 必需品
- It is necessary to do sth.
- unnecessary adj. 不需要的
- necessarily adv. 必然地
- necessity n. 必要
- entail
    - vt. 需要；牵涉； 
    - v. 使必要，需要 
    - n. 限定继承
    - To **entail** is to **involve**. A job at a movie theater might **entail** sweeping popcorn off the floor.
    - Deliberate practice **entails** more than simply repeating a task.

## source

- n. 来源；起源
- resource n. 资源
- resourceful adj. 资源丰富的，富于机智的，有策略的; 灵活多样，善于随机应变; 诡计多端的
- outsource
    - vt. 将...外包
    - The company began looking for ways to cut costs, which led to the decision to **outsource**.

## current

- adj. 现在的；通用的；流通的
- n. 水流；电流
- Some blame for the **current** controversy belongs to astronomers.
- currency 
    - n. 货币，通货
    - Yet the debate about how to save Europe's single **currency** from disintergration is stuck.
- currently adv. 当前
- occur
    - vi. 发生，出现
    - occurrence n. 发生地事情
- recur
    - vi. 再发生
    - Have you ever had the same crazy dream night after night? If so, you know that <ins>dreams can **recur**</ins> meaning they repeat, or happen again.
- incur 😔
    - vt. 引起；招致
    - If you don't pay your credit card bills on time, you'll likely **incur** lots of fees and some serious debt.
- up-to-date adj. 最新的，最近的

## depend

- vi. 取决于；依靠
- Depend on yourself; you won't be let down.
- depend on
- dependency n. 依赖
- dependent adj. 依赖的；取决于
- independence n. 独立；自力
    - independent adj. 独立的
- rely
    - vi. 依靠
    - == depend
    - rely on == depend on == count on == bank on 依赖
    - reliance n. 依赖
    - reliable adj. 可靠的
        - unreliable 
- lean vi. 倾向于； adj. 瘦且健康的；贫乏的
    - lean towards 倾向于

## negative

- adj. 负面的；否定的；阴性的
- positive
    - adj. 积极的
    - positively adv. 积极地；肯定地
- neutral
    - adj. 中性的；中立的；不动声色的
    - neutrality 
        - n. 中立（立场）；
        - == disinterest
        - net-neutrality law 网络中立法

## official

- adj. 官方的
- n. 官员
- executive 
    - adj. 经营管理的；行政的； 
    - n. 管理人员，领导层
    - Chief Executive Officer(CEO) 首席执行官
- authority
    - n. 权力；权威；官方机构；权威人士
    - authorization n. 授权
    - authoritative adj. 权威的
 - authentic
    - adj. 真实的；可靠的；正宗的
    - Kids need a range of **authentic** role models.

## responsibility

- n. 责任
- shoulder/assume/take/bear responsibility 承担责任
- take responsibility for 对...负责
- shrink the responsibility to do 逃避责任
- responsible 
    - adj. 有责任的；有责任心的；应受责备的
    - be responsible for 
- respond
    - v. （口头或书面）回答； 
    - vi. 做出反应
    - response n. （口头或书面）回答；反应
    - in response to 作为对...的回答
    - correspond
        - vi. 相一致；通信；类似于
        - correspond with/ to
        - correspondence n. 通信

## although

- conj. 虽然
- although A, B  尽管A，但是B（B是重点）
- unless conj. 除非
- whereas conj. （用以比较或对比两个事实）尽管；鉴于
- despite
    - prep. 尽管
    - **Despite** these factors, many social scientists seem reluctant to *tackle/deal with/cope with/handle/address* such problems.
- otherwise
    - adv. 否则；在其他方面
    - Failure can also teach us things about ourselves that we would never have learned **otherwise**.
- nevertheless adv. 然而
- nonetheless adv. 尽管如此

## Words

- distinguished
    - adj. 卓越的; 著名的; 受人尊敬的; 显得重要的
- cunning
    - adj. 狡猾的; 巧妙的
    - n. 狡猾，诡诈
- ultimate
    - adj. 最后的; 极限的; 首要的; 最大的
    - n. 终极; 顶点; 基本原理; 基本事实
    - == final
- renaissance
    - n. 文艺复兴（时期）; 复活，复燃
- ethics
    - n. 道德准则，行为准则; 伦理（学）（ ethic的名词复数 ）

## 词组

- redical viewpoints
    - 激进的观点
- casual style
    - 随意的风格
- practice makes perfect

## 句式

