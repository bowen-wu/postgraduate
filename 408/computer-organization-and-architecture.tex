...
\begin{document}
    ...
    \begin{enumerate}
        \item 数据寻址：如何在指令中表示一个操作数的地址，或怎样计算处操作数的地址
    ...
    \begin{table}[htbp]
        ...
        \begin{tabular}{|
                ...
                >{\centering\arraybackslash}m{0.10\textwidth}|}
            \hline
            & \textbf{含义} & \textbf{优点} & \textbf{缺点} & \textbf{备注} \\
            \hline
            隐含寻址 & 不明显地给出操作数的地址，而是隐含操作数的地址 & 有利于缩短指令字长 & 需要增加存储操作数或隐含地址的硬件 & \\
            \hline
            立即（数）寻址 & 指令字中的地址字段指出的是操作数本身，也称\textbf{立即数}，采用\textbf{补码}表示 & 指令在执行阶段不访存，指令执行速度最快 & $A$的位数限制了立即数的范围 & 特征：$\#$ \\
            \hline
            直接寻址 & 指令字中的形式地址$(A)$就是操作数的真实地址$(EA)$ & 简单，指令在执行阶段仅需访存一次 & $A$的位数限制了该指令操作数的寻址范围，操作数的地址不易修改 & \\
            \hline
            间接寻址 & 指令的地址字段是操作数有效地址所在主存单元的地址，即操作数地址的地址 & 可扩大寻址范围，便于编制程序（用间接寻址可方便地完成子程序返回） & 指令执行阶段要多次访存（$n$次间接寻址需$n+1$次访存）& \\
            \hline
            寄存器寻址 & 指令字中的地址字段给出的是操作数所在寄存器的编号 & 执行阶段不用访存，执行速度快；寄存器数量远小于内存单元数，所以地址码位数较少，指令字长较短 & CPU的寄存器数量有限，寻址能力有限 & \\
            \hline
            寄存器间接寻址 & 指令字中的$R_i$所指寄存器给出操作数所在主存单元的地址 & 扩大了寻址范围，减少了访存次数 & & \\
            \hline
            相对寻址 & 将PC的内容加上指令格式中的形式地址$(A)$而形成操作数的有效地址，其中$(A)$是相对于\textbf{下一条}PC值的偏移量，\textbf{补码}表示 & 操作数的地址随PC值的变化而变化，且与指令地址之间总是相差一个固定的偏移量，因此便于程序浮动（{\color{customgreen}{一段代码在程序内部的浮动}}） && 广泛应用于\textbf{转移指令}  \\
            \hline
            基址寻址 & 将基址寄存器$(BR)$的内容加上指令字中的形式地址$(A)$而形成操作数的有效地址                                      & 扩大寻址范围（基址寄存器的位数大于形式地址$(A)$的位数）；有利于多道程序设计，并可用于编制浮动程序（{\color{customgreen}{整段代码在内存中浮动}}） & & 面向系统，用于为多道程序或数据分配存储空间  \\
            \hline
            变址寻址 & 将变址寄存器$(IX)$的内容加上指令字中的形式地址$(A)$而形成操作数的有效地址                                      & 扩大寻址范围；适合编制循环程序                                                                        & & 立足于用户，主要用于处理数组问题，变址寄存器的内容有用户设定  \\
            \hline
            堆栈寻址 & & & & \\
            \hline
        \end{tabular}
