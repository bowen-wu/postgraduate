...
% =========================
    ...
    考研中高频“模板型”（12+1）：
    \begin{enumerate}
        \item 二阶行列式：$\begin{vmatrix}a&b\\c&d\end{vmatrix}=ad-bc$。
        ...
        \item 分块上（下）三角行列式：$\left|\begin{matrix}A&*\\O&B\end{matrix}\right|=|A||B|$。
        \item 分块副对角形式：$\left|\begin{matrix}*&A\\B&O\end{matrix}\right|=(-1)^{mn}|A||B|$。
    ...
    二阶分块常用结论：
    \[
        \left|\begin{matrix}A & *\\ O & B\end{matrix}\right|=|A||B|,\qquad
        \left|\begin{matrix}* & A\\ B & O\end{matrix}\right|=(-1)^{mn}|A||B|
    \]
    ...
    \begin{enumerate}
        ...
        \item 函数与方程：把某行（列）视为参数，利用线性性先设$f(x)$再代值定系数。
    \end{enumerate}
