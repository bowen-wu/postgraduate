...
\begin{document}
    ...
    \begin{enumerate}
        ...
        \begin{itemize}
            ...
            \begin{circlenum}
                ...
                \item 某一路径使极限不存在
            \end{circlenum}
            \item 除{\color{red}{\sout{洛必达法则}}}和{\color{red}{\sout{单调有界准则}}}外，二重极限保持了一元极限的唯一性、局部有界性、局部保号性、运算规则、脱帽法和{\color{red}{夹逼准则}}
        \end{itemize}
    ...
    \begin{enumerate}
        ...
        即
        \[
            f'_x(x_0, y_0) = \lim_{\Delta x \to 0} \frac{f(x_0 + \Delta x, y_0) - f(x_0, y_0)}{\Delta x} = \lim\limits_{x\to x_0} \frac{f(x, y_0) - f(x_0, y_0)}{x - x_0}
        \]
        类似地，函数 $z = f(x,y)$ 在点 $(x_0,y_0)$ 处对 $y$ 的\textbf{偏导数}定义为
        \[
            f'_y(x_0, y_0) = \lim_{\Delta y \to 0} \frac{f(x_0, y_0 + \Delta y) - f(x_0, y_0)}{\Delta y} = \lim\limits_{y\to y_0} \frac{f(x_0, y) - f(x_0, y_0)}{y - y_0}
        \]
        ...
        \item \textbf{混合偏导数相等的充分条件}：如果 $z = f(x,y)$ 的两个二阶混合偏导数 $\dfrac{\partial^2 z}{\partial x \partial y}$ 和 $\dfrac{\partial^2 z}{\partial y \partial x}$ 在区域 $D$ 内{\color{red}{连续}}，那么在该区域内这两个二阶混合偏导数必相等。即二阶混合偏导数在连续的条件下与求导的次序无关
    \end{enumerate}
    ...
    \begin{enumerate}
        ...
        \textbf{多元函数不可逆的原因}：偏导数只反映沿坐标轴方向的变化率，不能保证{\color{red}{沿任意方向}}都能用线性函数近似。
        ...
        \begin{enumerate}
            ...
            \end{circlenum}
            \item \includegraphics[width=\linewidth]{\subfix{../../../assets/math/advanced-mathematics/pic13-6.png}}
        \end{enumerate}

        \item \textbf{可微的判别步骤}：
        \begin{circlenum}
            \item 写出全增量$\Delta z = f(x_0 + \Delta x, y_0 + \Delta y) - f(x_0, y_0)$
            \item 写出线性增量$A\Delta x + B\Delta y$，其中$A = f'_x(x_0, y_0) = \displaystyle\frac{\partial f}{\partial x}\bigg|_{\substack{x=x_0 \\ y=y_0}}, B = f'_y(x_0, y_0) = \displaystyle\frac{\partial f}{\partial y}\bigg|_{\substack{x=x_0 \\ y=y_0}}$。偏导数不好求时，可以用导数概念求$A, B$
            \item 做极限
            \[
    ...
    \begin{enumerate}
        ...
        \begin{center}
            ...
            \end{minipage}
        \end{center}
        \item $\color{red}{\bigstar}$\textbf{结构不变性}：无论$z$对哪个变量求导，也无论$z$已经求了几阶导，求导后的新函数仍然具有与原函数完全相同的复合结构

        \textbf{理解}：设 $z = f(u,v)$，其中 $u = \varphi(x,y)$，$v = \psi(x,y)$。

        根据链式法则，$\dfrac{\partial z}{\partial x} = \dfrac{\partial f}{\partial u}\dfrac{\partial u}{\partial x} + \dfrac{\partial f}{\partial v}\dfrac{\partial v}{\partial x}$。

        这里 $\dfrac{\partial f}{\partial u}$ 和 $\dfrac{\partial f}{\partial v}$ \textbf{仍然是 $u, v$ 的函数}，而 $u, v$ 仍然是 $x, y$ 的函数。因此 $\dfrac{\partial z}{\partial x}$ 依然通过 $u, v$ 与 $x, y$ 相关联——这就是{\blueuline{复合结构不变}}。

        \textbf{意义}：求高阶偏导时，每次都要继续用链式法则处理中间变量的依赖关系。

        \begin{analysisbox}[Example]
            设 $z = f(u,v)$，其中 $u = \varphi(x,y)$，$v = \psi(x,y)$，求 $\dfrac{\partial^2 z}{\partial x^2}$。
            ...
            注意：$f'_u$ 和 $f'_v$ \textbf{仍是 $u, v$ 的函数}，复合结构未变。

            \textbf{二阶偏导}：对 $f'_u \cdot \varphi'_x + f'_v \cdot \psi'_x$ 再求 $\dfrac{\partial}{\partial x}$ 时，\textbf{仍需用链式法则}：
            \begin{align*}
                \frac{\partial^2 z}{\partial x^2} &= \frac{\partial}{\partial x}(f'_u \cdot \varphi'_x) + \frac{\partial}{\partial x}(f'_v \cdot \psi'_x) \\
                &= \left(f''_{uu} \cdot \varphi'_x + f''_{uv} \cdot \psi'_x\right)\varphi'_x + f'_u \cdot \varphi''_{xx} \\
                &\quad + \left(f''_{vu} \cdot \varphi'_x + f''_{vv} \cdot \psi'_x\right)\psi'_x + f'_v \cdot \psi''_{xx}
            \end{align*}

            \textbf{关键}：求 $\dfrac{\partial}{\partial x}(f'_u)$ 时，$f'_u$ 仍要通过 $u, v$ 对 $x$ 求导。
        \end{analysisbox}
        \item 记号（设$z = f(\boxed{\text{第1个输入}},\boxed{\text{第2个输入}})$)
        \begin{itemize}
            \item $f'_1 == f'_1(\boxed{\text{第1个输入}},\boxed{\text{第2个输入}}) = \dfrac{\partial f}{\partial \boxed{\text{第1个输入}}}$ 表示对第一个位置求导
            \item $f'_2 == f'_2(\boxed{\text{第1个输入}},\boxed{\text{第2个输入}}) = \dfrac{\partial f}{\partial \boxed{\text{第2个输入}}}$ 表示对第二个位置求导
        \end{itemize}
        \begin{analysisbox}[Example]
            $f(x, y) \Rightarrow f'_x = \dfrac{\partial f}{\partial x}$ 表示对第一个位置求导 \\
            $f(u, v) \Rightarrow f'_v = \dfrac{\partial f}{\partial v}$ 表示对第一个位置求导
        \end{analysisbox}
    ...
    \begin{analysisbox}[Example]
        设 $z = e^{u}\sin v$，其中 $u = x^2 y$，$v = x + y^2$，求 $\dfrac{\partial z}{\partial x}$ 和 $\dfrac{\partial z}{\partial y}$。

        \textbf{方法一}：链式法则
        \begin{align*}
            \frac{\partial z}{\partial x} &= \frac{\partial z}{\partial u}\frac{\partial u}{\partial x} + \frac{\partial z}{\partial v}\frac{\partial v}{\partial x} = e^{u}\sin v \cdot 2xy + e^{u}\cos v \cdot 1 \\
            &= e^{x^2 y}(2xy\sin(x+y^2) + \cos(x+y^2))\\[1em]
            \frac{\partial z}{\partial y} &= \frac{\partial z}{\partial u}\frac{\partial u}{\partial y} + \frac{\partial z}{\partial v}\frac{\partial v}{\partial y} = e^{u}\sin v \cdot x^2 + e^{u}\cos v \cdot 2y \\
            &= e^{x^2 y}(x^2\sin(x+y^2) + 2y\cos(x+y^2))
        \end{align*}

        \textbf{方法二}：全微分形式不变性
        \begin{align*}
            \mathrm{d}z &= e^{u}\sin v\,\mathrm{d}u + e^{u}\cos v\,\mathrm{d}v \\
            &= e^{u}\sin v(2xy\,\mathrm{d}x + x^2\,\mathrm{d}y) + e^{u}\cos v(\mathrm{d}x + 2y\,\mathrm{d}y) \\
            &= e^{u}\big[\sin v \cdot 2xy + \cos v\big]\mathrm{d}x + e^{u}\big[\sin v \cdot x^2 + \cos v \cdot 2y\big]\mathrm{d}y
        \end{align*}
        \textbf{一次得到两个结果}：
        \[
        ...
        \textbf{优势}：全微分形式不变性一次运算同时得到 $\dfrac{\partial z}{\partial x}$ 和 $\dfrac{\partial z}{\partial y}$，且避免了重复计算 $\dfrac{\partial z}{\partial u}$ 和 $\dfrac{\partial z}{\partial v}$
    \end{analysisbox}
    ...
    {\linespread{1.15}\selectfont
        \begin{enumerate}
            \item \textbf{隐函数存在定理1}（一个方程，两个变量）：对于由方程$F(x, y) = 0$确定的隐函数$y = f(x)$，当$\color{red}{F_y(x_0, y_0) \neq 0 \text{（隐函数存在条件）}}$时，则有
            \[
                \frac{\mathrm{d}y}{\mathrm{d}x} = {\color{red}{-}}\frac{F'_x(x, y)}{F'_y(x, y)}
            \]

            \item \textbf{隐函数存在定理2}（一个方程，三个变量）：对于由方程$F(x, y, z) = 0$确定的隐函数$z = f(x, y)$，当$\color{red}{F_z(x_0, y_0, z_0) \neq 0 \text{（隐函数存在条件）}}$时，则有
            \[
    ...
    \begin{enumerate}
        ...
        \[
            \begin{cases}
                \text{极大值} \;\Rightarrow\;
                f''_{xx}(x_0, y_0) \le 0,\quad
                f''_{yy}(x_0, y_0) \le 0, \\[6pt]
                \text{极小值} \;\Rightarrow\;
                f''_{xx}(x_0, y_0) \ge 0,\quad
                f''_{yy}(x_0, y_0) \ge 0.
            \end{cases}
        \]
        ...
        \begin{itemize}
            ...
            \item 找{\color{red}{可疑点}}\begin{cases}
    ...
    求目标函数$u = f(x, y, z)$在约束条件\begin{cases}
                                            \varphi(x, y, z) = 0, \\
                                            \psi(x, y, z) = 0
    \end{cases}下的最值，则(在满足约束方程的那些点里，找极值)
    \begin{circlenum}
        ...
        \item 注意：
        \begin{itemize}
            \item \textbf{拉格朗日法的求解对象}：拉格朗日法求的是约束条件上的驻点（极值候选点），上述驻点为约束曲线（或曲面）上的\textbf{非端点点}。若约束不封闭或存在端点，还需\textbf{比较端点处函数值}。
            \item 若目标函数的约束条件为$\varphi(x, y, z) = 0$，易解出$z = z(x, y)$，则将其代入$f(x, y, z)$，的$f[x, y, z(x, y)]$，即转化为无条件最值问题
        \end{itemize}
    \end{circlenum}
    ...
    \begin{enumerate}
        ...
        \begin{minipage}{0.15\textwidth}
            \centering
            \includegraphics[width=\linewidth]{\subfix{../../../assets/math/advanced-mathematics/P1P2Q.png}}
        \end{minipage}
        ...
        \begin{minipage}{0.15\textwidth}
            \centering
            \includegraphics[width=\linewidth]{\subfix{../../../assets/math/advanced-mathematics/P1P2.png}}
        \end{minipage}
    \end{enumerate}
    ...
    \begin{enumerate}
        ...
        \item \textbf{求解步骤}：
        \begin{circlenum}
            \item 内部（无约束极值）：根据$f'_x(x, y), f'_y(x, y)$为$0$或不存在，求出{\color{red}{区域$D$内部}}的所有可疑点
            \item 边界（有约束极值）：用拉格朗日乘数法或代入法求出{\color{red}{区域$D$边界上}}的所有可疑点
            \begin{itemize}
                \item 若约束不封闭或存在端点，还需\textbf{比较端点处函数值}
                \item 拉格朗日的约束 == 边界方程
            \end{itemize}
    ...
    \begin{enumerate}
        ...
        \item \begin{minipage}{0.55\textwidth}
                  $\displaystyle\int_0^{+\infty} e^{-x^2}\,\mathrm{d}x = \frac{\sqrt {\pi}}{2}$
        \end{minipage}%
        \hfill
        \begin{minipage}{0.4\textwidth}
            \centering
            \begin{tikzpicture}
                \begin{axis}
                    ...
                    % 标注曲线函数
                    \node at (axis cs:1.8,0.75) {$y=e^{-x^2}$};
                \end{axis}
            \end{tikzpicture}
        \end{minipage}
    \end{enumerate}
    ...
    \begin{enumerate}
        \item 偏导求法
        \begin{itemize}
            \item 一点处的偏导 $\Rightarrow$ 定义法
            \item 某区域$D$的偏导 $\Rightarrow$ 公式法
            \item 表达式太复杂 $\Rightarrow$ 换元 + 链式法则
        \end{itemize}
        \item $f'_x(x_0, y_0)$ = \begin{cases}
                                     f'_x(x, y_0)\Big|_{x=x_0}, \text{先代值再求导} \\
                                     f'_x(x, y)\Big|_{x = x_0, y = y_0} \text{先求导再代值}
        \end{cases}
        \item $\displaystyle\int_a^b |x| \mathrm{d}x = \dfrac{1}{2}(a \leq 0, b \geq 0)$，由于绝对值函数在 $x=0$ 处不可导，计算定积分时需按分段函数处理$\displaystyle\int_a^b |x| \mathrm{d}x = \int_a^0 -x \mathrm{d}x + \int_0^b x \mathrm{d}x$
        \item 遇到绝对值要分类讨论
        \item 在选项中要注意自变量的取值范围。\emph{e.g.} $x^2$ vs. $\dfrac{1}{x^2}, x\neq 0$，可排除选项
    \end{enumerate}

    \subsection{方法步骤}

    \begin{enumerate}
        \item 反问题：已知$f'_x, f'_y$，求$f(x, y)$
        \begin{analysisbox}[Example]
            已知
            \[
            ...
            对 $y$ 积分得
            \[
                f(x,y)=\int e^{-x}\cos y\,dy
                =e^{-x}\sin y+\varphi(x),
            \]

            其中 $\varphi(x)$ 为关于 $x$ 的任意函数（关于$y$的常数），因为$\dfrac{\partial}{\partial y}\varphi(x)=0.$
        \end{analysisbox}
        \item 设曲线由方程$x^2 + 4y^2 = 4$确定，求 $\dfrac{\mathrm{d}y}{\mathrm{d}x}$。
        ...
        \end{analysisbox}
        \item 思考：若$AC - B^2 = 0$该如何处理
        \begin{analysisbox}
            问：求函数$f(x, y) = x^2(2x^2 - 4y - \dfrac{1}{7}x^5) + y^2$的极值
            解：令\begin{cases}
                     f_x^'(x,y) = 8x^3 - 8xy - x^6 = 0 \\
                     f_y^'(x,y) = -4x^2 + 2y = 0
            \end{cases}得驻点$(0, 0), (-2, 8)$且
            $f_{xx}^{''} = 24x^2 - 8y - 6x^5, f_{xy}^{''} = -8x, f_{yy}^{''} = 2$
            由$A = f_{xx}^{''}(0, 0) = 0, B = f_{xy}^{''}(0, 0) = 0, C = f_{yy}^{''}(0, 0) = 2$，得$AC-B^2 = 0$，又在点$(0, 0)$附近
            $f(x, x^2) = -\dfrac{1}{7}x^7 - x^4 = -x^4(\dfrac{1}{7}x^3 + 1) < 0, f(0, y) = y^2 > 0(y \neq 0)$，
            且$f(0, 0) = 0$，故$(0, 0)$不是极值点
            由$A = f_{xx}^{''}(-2, 8) = 224, B = f_{xy}^{''}(-2, 8) = 16, C = f_{yy}^{''}(-2, 8) = 2$，得$AC-B^2 > 0$，又$A = 224 > 0$，因此
            $(-2, 8)$为极小值点，极小值为$f(-2, 8) = -\dfrac{96}{7}$，无极大值
        \end{analysisbox}
    \end{enumerate}
