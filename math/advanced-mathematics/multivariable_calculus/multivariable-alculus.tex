...
\begin{document}
    ...
    \begin{enumerate}
        ...
        这里 $\dfrac{\partial f}{\partial u}$ 和 $\dfrac{\partial f}{\partial v}$ \textbf{仍然是 $u, v$ 的函数}，而 $u, v$ 仍然是 $x, y$ 的函数。因此 $\dfrac{\partial z}{\partial x}$ 依然通过 $u, v$ 与 $x, y$ 相关联——这就是"复合结构不变"。
    ...
    \begin{enumerate}
        ...
        \[
            \begin{cases}
                \text{极大值} \;\Rightarrow\;
                f_{xx}(x_0, y_0) \le 0,\quad
                f_{yy}(x_0, y_0) \le 0, \\[6pt]
                \text{极小值} \;\Rightarrow\;
                f_{xx}(x_0, y_0) \ge 0,\quad
                f_{yy}(x_0, y_0) \ge 0.
            \end{cases}
        \]
    ...
    \begin{enumerate}
        ...
        \item \textbf{求解步骤}：
        \begin{circlenum}
            \item 根据$f'_x(x, y), f'_y(x, y)$为$0$或不存在，求出{\color{red}{区域$D$内部}}的所有可疑点
            \item 用拉格朗日乘数法或代入法求出{\color{red}{区域$D$边界上}}的所有可疑点
