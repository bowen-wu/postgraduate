...
\begin{document}
    ...
    设函数$f(x)$定义在某区间$I$上，若存在可导函数$F(x)$，对于该区间上任意一点都有$F'(x) = f(x)$成立，则称$F(x)$是$f(x)$在区间$I$上的一个\textbf{原函数}。称$\dint f(x)\mathrm{d}x = F(x) + C$为$f(x)$在区间$I$上的\textbf{不定积分（全体原函数）}
    ...
    \begin{enumerate}
        ...
        \[
                {\color{red}{\bigstar}}\, f(x)\text{ 连续}
            \Rightarrow
            \left\{
                \begin{aligned}
                    &{\color{red}{\int f(x)\,\mathrm{d}x = \int_a^x f(t)\,\mathrm{d}t + C}},\\[6pt]
    ...
    \begin{enumerate}
        ...
        则称函数 $f(x)$ 在区间 $[a,b]$ 上{\textbf{\color{red}{可积}}}，即
        \[
        ...
        \[
                {\color{red}{\bigstar\bigstar}}\int_0^1 f(x)\,\mathrm{d}x
            =
            \lim_{n \to \infty}
            \sum_{i=1}^{n}
            ...
            \frac{1}{n}, \quad\text{中点：}\frac{\displaystyle\frac{i - 1}{n} + \frac{i}{n}}{2}
        \]
    ...
    \begin{enumerate}
        \item 定积分存在的充分条件
        \begin{circlenum}
            \item 若$f(x)$在$[a, b]$上连续，则$\dint_a^b f(x)\,\mathrm{d}x$存在
            \item 若$f(x)$在$[a, b]$上单调（{\color[rgb]{0.2, 0.6, 0.3}{$f(a), f(b)$即为函数的界，对任意$x\in (a, b)$，$f(x)$一定介于$f(a), f(b)$之间}}），则$\dint_a^b f(x)\,\mathrm{d}x$存在
            \item 若$f(x)$在$[a, b]$上有界，且只有有限个间断点（{\color[rgb]{0.2, 0.6, 0.3}{不包含无穷间断点}}），则$\dint_a^b f(x)\,\mathrm{d}x$存在
            \item 若$f(x)$在$[a, b]$上有有限个第一类间断点，则$\dint_a^b f(x)\,\mathrm{d}x$存在
        \end{circlenum}
        \item 定积分存在的必要条件: 可积函数必有界。即若定积分$\dint_a^b f(x)\,\mathrm{d}x$存在，则$f(x)$在$[a, b]$上必有界
    \end{enumerate}

    \subsubsection{性质（假设以下积分均存在）}

    \begin{enumerate}
        \item 当$b = a$时，$\dint_a^b f(x)\,\mathrm{d}x = 0$
        \item 当$a > b$时，$\dint_a^b f(x)\,\mathrm{d}x = -\dint_b^a f(x)\,\mathrm{d}x$
        \item 求区间长度：假设$a<b$，则$\dint_a^b \mathrm{d}x = b-a = L$，其中$L$为区间$[a, b]$的长度
        \item 积分的线性性质：设$k_1, k_2$为常数，则$\dint_a^b [k_{1}f(x)\pm k_{2}g(x)]\,\mathrm{d}x = k_{1}\dint_a^b f(x)\,\mathrm{d}x \pm k_{2}\dint_a^b g(x)\,\mathrm{d}x$
        \item 积分的可加（拆）性：无论$a, b, c$的大小如何，总有$\dint_a^b f(x)\,\mathrm{d}x = \dint_a^c f(x)\,\mathrm{d}x + \dint_c^b f(x)\,\mathrm{d}x$
        \item 积分的保号性：若在区间$[a, b]$上$f(x) \leq g(x)$，则有$\dint_a^b f(x)\,\mathrm{d}x \leq \dint_a^b g(x)\,\mathrm{d}x$。特殊地，有
        \[
        ...
        \]
        \item $\color{red}{\bigstar}$积分中值定理：若函数$f(x)$在区间$[a, b]$上连续，则至少存在一点$\xi \in [a, b]$，使
        \[
    ...
    当 $x$ 在区间 $[a,b]$ 上变动时，对应于每一个 $x$ 值，积分$\dint_a^x f(t)\,\mathrm{d}t$都有一个确定的值，因此$\dint_a^x f(t)\,\mathrm{d}t$是一个关于$x$的函数，记作
    \[
    ...
    \begin{enumerate}
        \item 函数$f(x)$在$I$上\textbf{可积}，则函数$F(x) = \dint_a^x f(t)\,\mathrm{d}t$在$I$上\textbf{连续}（$F(x)$若存在，则其一定连续。与$f(x)$作区分：$f(x)$存在但其不一定连续)
        \begin{analysisbox}[证（理解）：]
            ...
            只要它存在，就必然是连续的
        \end{analysisbox}
        \item $\color{red}{\bigstar}$函数$f(x)$在$I$上\textbf{连续}，则函数$F(x) = \dint_a^x f(t)\,\mathrm{d}t$在$I$上可导且$F'(x) = f(x)$
        \item 设 $F(x)=\dint_a^x f(t)\,\mathrm{d}t$，则有：
        \begin{itemize}
    ...
    \begin{enumerate}
        ...
        \end{circlenum}
        \item 瑕点：使$f(x)$在$x_0$的邻域内\textbf{\color{red}{无界}}的点即为\textbf{瑕点}。如：
        \begin{itemize}
        ...
        \end{itemize}
        \item \textbf{无界函数}的反常积分的概念与敛散性：设$F(x)$是$f(x)$在相应区间上的一个原函数，{\color{red}{$x_0$为$f(x)$的瑕点}}
        \begin{circlenum}
            \item 若$x = a$是唯一瑕点，则
            \[
    ...
    \begin{enumerate}
        ...
        \begin{itemize}
            ...
            \begin{circlenum}
                \item 当$\dint_a^{+\infty} g(x)\,\mathrm{d}x$收敛时，$\dint_a^{+\infty} f(x)\,\mathrm{d}x$收敛
                \item 当$\dint_a^{+\infty} f(x)\,\mathrm{d}x$发散时，$\dint_a^{+\infty} g(x)\,\mathrm{d}x$发散
            \end{circlenum}
            ...
            \begin{circlenum}
                \item 当$\lambda \neq 0$且$\lambda \neq \infty$时（\textbf{$f(x), g(x)$同阶无穷小}），$\dint_a^{+\infty} f(x)\,\mathrm{d}x$与$\dint_a^{+\infty} g(x)\,\mathrm{d}x$有相同的敛散性
                \item 当$\lambda = 0$时（\textbf{$f(x)$趋于$0$的速度比$g(x)$更快}），若$\dint_a^{+\infty} g(x)\,\mathrm{d}x$收敛，则$\dint_a^{+\infty} f(x)\,\mathrm{d}x$收敛
                \item 当$\lambda = \infty$时（\textbf{$g(x)$趋于$0$的速度比$f(x)$更快}），若$\dint_a^{+\infty} g(x)\,\mathrm{d}x$发散，则$\dint_a^{+\infty} f(x)\,\mathrm{d}x$发散
            \end{circlenum}
        \end{itemize}
        \item 无界函数
        \begin{itemize}
            ...
            \begin{circlenum}
                \item 当$\dint_a^{b} g(x)\,\mathrm{d}x$收敛时，$\dint_a^{b} f(x)\,\mathrm{d}x$收敛
                \item 当$\dint_a^{b} f(x)\,\mathrm{d}x$发散时，$\dint_a^{b} g(x)\,\mathrm{d}x$发散
            \end{circlenum}
            ...
            \begin{circlenum}
                \item 当$\lambda \neq 0$且$\lambda \neq \infty$时（\textbf{$f(x), g(x)$同阶无穷大}），$\dint_a^{b} f(x)\,\mathrm{d}x$与$\dint_a^{b} g(x)\,\mathrm{d}x$有相同的敛散性
                \item 当$\lambda = 0$时（\textbf{$g(x)$趋于$\infty$的速度比$f(x)$更快}），若$\dint_a^{b} g(x)\,\mathrm{d}x$收敛，则$\dint_a^{b} f(x)\,\mathrm{d}x$收敛
                \item 当$\lambda = \infty$时（\textbf{$f(x)$趋于$\infty$的速度比$g(x)$更快}），若$\dint_a^{b} g(x)\,\mathrm{d}x$发散，则$\dint_a^{b} f(x)\,\mathrm{d}x$发散
            \end{circlenum}
        \end{itemize}
        \item 四个重要结论（$p$ 型广义积分）
        \begin{circlenum}
            ...
            \]
            盯着$x\to +\infty$看
            \item 当$x\to 0^+$时，$\sin x \sim x$，即$\lim\limits_{x\to 0^+} \dfrac{\sin^p x}{x^p} = 1$，故$\dint_0^1 \dfrac{1}{\sin^p x}\,\mathrm{d}x$依然满足\ding{172}
            \item 若$f(x)\sim x^\alpha \quad (x\to 0^+ \text{ 或 } +\infty)$，则$\dint \dfrac{1}{f^p(x)}\,\mathrm{d}x \sim \dint \dfrac{1}{x^{\alpha p}}\,\mathrm{d}x$
        \end{circlenum}
        \item 步骤
        \begin{circlenum}
            \item 若$\lim\limits_{x\to \infty} f(x) = \infty$，则$\dint_a^\infty f(x)\,\mathrm{d}x$ 发散
            \item 找到积分上下限中的所有{\color{red}{奇点}}($\infty$ + 瑕点)
            \item 拆分：保证每个积分中只有一个奇点
            \item 放缩法或计算极限
            \begin{enumerate}
                \item 放缩法：$0 \geq f(x) \geq g(x)$
                \item 计算极限：计算奇点处的极限$\lim\limits_{x\to \text{奇点}} \dfrac{f(x)}{\text{辅助函数}}$
                \item 辅助函数：$p$ 型广义积分
            \end{enumerate}
        \end{circlenum}
    \end{enumerate}
    ...
    \begin{enumerate}
        ...
        \item $\sin x$和$\cos x$ 一拱面积($(\dint_0^{\pi} \sin x\,\mathrm{d}x$)为2
        \item $\int_0^1 \sqrt{1 - x^2}\,\mathrm{d}x$等于单位圆在第一象限的面积，即$\dfrac{\pi}{4}$
        \item $\dint_0^{\frac{\pi}{2}} f(\sin x)\,\mathrm{d}x == \dint_0^{\frac{\pi}{2}} f(\cos x)\,\mathrm{d}x$。令$x = \dfrac{\pi}{2} - t$
        \item 奇函数和偶函数两侧敛散性相同
        \item 当$f(x)$是偶函数且$\dint_0^{+\infty} f(x)\,\mathrm{d}x$收敛时，$\dint_{-\infty}^{+\infty} f(x)\,\mathrm{d}x = 2\dint_0^{+\infty} f(x)\,\mathrm{d}x$
        \item 当$f(x)$是奇函数且$\dint_0^{+\infty} f(x)\,\mathrm{d}x$收敛时，$\dint_{-\infty}^{+\infty} f(x)\,\mathrm{d}x = 0$
        \item 设$f(x)$连续，且$\dint_a^{+\infty} |f(x)|\,\mathrm{d}x$收敛，则$\dint_a^{+\infty} f(x)\,\mathrm{d}x$收敛
        \begin{analysisbox}[证:]
            因为
            \[
    ...
    \begin{enumerate}
        \item 见到 $\dint_a^b f(x)\,\mathrm{d}x$
        \[
            \begin{cases}
                \text{用积分中值定理：}
                & \dint_a^b f(x)\,\mathrm{d}x = f(\xi)(b-a), \\[6pt]
                \text{改成辅助函数：}
                & \dint_a^x f(t)\,\mathrm{d}t.
            \end{cases}
        \]
    \end{enumerate}
    ...
    \begin{enumerate}
        ...
        \item $\dint_0^1 \ln x\,\mathrm{d}x$和$\dint_{-\infty}^0 e^x\,\mathrm{d}x$ 是收敛的（$\color{red}{\lim\limits_{x\to 0^+} x^{\epsilon}\ln x = 0, \epsilon > 0}$），且
        \[
            \int_0^1 \ln x\,\mathrm{d}x = -\int_{-\infty}^0 e^x\,\mathrm{d}x = -1
        \]
    \end{enumerate}
    ...
    \begin{enumerate}
        \item $\dint f(x)\,\mathrm{d}x$称为不定积分，表示{\color{red}{全体原函数}}
        \item $\color{red}{\bigstar}\color{red}{\bigstar}$不定积分对应一族原函数，而变限积分给出了其中一个唯一确定的原函数。当需要研究函数的{\color{性态}}（如极限、导数、单调性等）时，必须将不定积分转化为变限积分
        \item
        \begin{analysisbox}
            \textbf{不定积分、定积分与原函数的关系}
            ...
            \[
                \boxed{
                    ...
                    y(x_0)
                    +
                    \int_{x_0}^{x} y'(t)\,\mathrm{d}t.
                }
            \]
        \end{analysisbox}
        \item 变限积分的作用不是“换一种写法”，而是“选定一个原函数”
        \item $\dint_a^b f(x)\,\mathrm{d}x$称为定积分，表示面积
        \item $F(x) = \dint_a^x f(t)\,\mathrm{d}t$称为变上限积分，表示动态的面积
        \item 设$f(x)$在$(0, 1)$上单调，若反常积分$\dint_0^1 f(x)\,\mathrm{d}x$收敛，则$\lim\limits_{n\to \infty} \dfrac{f(\dfrac{1}{n}) + \cdots + f(\dfrac{n - 1}{n})}{n} = \dint_0^1 f(x)\,\mathrm{d}x$
        \item 证明：如果函数$f(x)$在$[a, b]$上连续，则函数$F(x) = \dint_a^x f(t)\,\mathrm{d}t$在$[a, b]$上可导，且$F'(x) = f(x)$，即$\color{red}{\dint f(x)\,\mathrm{d}x = \dint_a^x f(t)\,\mathrm{d}t + C}$
        \begin{analysisbox}[证]
            ...
            由积分中值定理，有$\dint_x^{x + \Delta x} f(t)\,\mathrm{d}t = f(\xi)\Delta x$，其中 $\xi$ 介于 $x$ 与 $x+\Delta x$ 之间，当$\Delta x\to 0$时，$\xi \to x$，故
            \[
        ...
        \end{analysisbox}
        \item 设$f(x)$是$[a, b]$上{\color{red}{非负}}的连续函数，且$f(x)${\color{red}{不恒等于零}}，证明必有$\dint_a^b f(x)\,\mathrm{d}x > 0$
        \begin{analysisbox}[证]
