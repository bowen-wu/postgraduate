...
\begin{document}
    ...
    \begin{enumerate}
        ...
        \item $\sin x$和$\cos x$ 一拱面积($(\int_0^{2\pi} \sin x\,\mathrm{d}x$)为2
    ...
    \begin{enumerate}
        ...
        \item $\int_0^1 \ln x\,\mathrm{d}x$和$\int_{-\infty}^0 e^x\,\mathrm{d}x$ 是收敛的（$\color{red}{\lim\limits_{x\to 0^+} x^{\epsilon}\ln x = 0, \epsilon > 0}$），且
        \[
    ...
    \begin{enumerate}
        \item $\displaystyle \int f(x)\,\mathrm{d}x$称为不定积分，表示{\color{red}{全体原函数}}
        \item $\color{red}{\bigstar}\color{red}{\bigstar}$不定积分对应一族原函数，而变限积分给出了其中一个唯一确定的原函数。当需要研究函数的性态（如极限、导数、单调性等）时，必须将不定积分转化为变限积分
        \item 变限积分的作用不是“换一种写法”，而是“选定一个原函数”
        \item $\displaystyle \int_a^b f(x)\,\mathrm{d}x$称为定积分，表示面积
        \item $\displaystyle F(x) = \int_a^x f(t)\,\mathrm{d}t$称为变上限积分，表示动态的面积
        ...
        \end{analysisbox}
        \item 设$f(x)$是$[a, b]$上{\color{red}{非负}}的连续函数，且$f(x)${\color{red}{不恒等于零}}，证明必有$\int_a^b f(x)\,\mathrm{d}x > 0$
        \begin{analysisbox}[证]
