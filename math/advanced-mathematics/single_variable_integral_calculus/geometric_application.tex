...
\begin{document}
    ...
    \begin{enumerate}
        \item 曲线$y = y(x)$与$x = a, x = b(a < b)$及$x$轴围成的曲边梯形绕$x$轴旋转一周所得到的的旋转体（圆柱体：$\pi r^2$）的体积
        \[
            V_x = \int_a^b \pi y^2(x)\,\mathrm{d}x
        \]
        \item 曲线$y = y(x)$与$x = a, x = b(0 \leq a < b)$及$x$轴围成的曲边梯形绕$y$轴旋转一周所得到的的旋转体（圆柱壳体积：$2\pi x|y| dx$(圆柱侧面积(底面周长 * 高) * 壳厚)）的体积
        \[
    ...
    {\color{customgreen}{$\mathrm{d}s = \sqrt{\mathrm{d}x^2 + \mathrm{d}y^2}$}}
    ...
    {\color{customgreen}{旋转曲面的侧面积 = 圆周长($2\pi r$) × 微元弧长}}
    \begin{circlenum}
        \item 绕$x$轴：$r = |y|$
        \item 绕$y$轴：$r = |x|$
        \item $S=2\pi\dint(\text{到旋转轴的距离})\,\mathrm{d}s$
    \end{circlenum}
...
% 最高点
    ...
    \begin{enumerate}
        \item 等比公式前$n$项和: $S_n = \dfrac{a_1(1-r^n)}{1-r} = \dfrac{a_{1}r^n - a_1}{r - 1}, \quad r \neq 1 $
    \end{enumerate}
