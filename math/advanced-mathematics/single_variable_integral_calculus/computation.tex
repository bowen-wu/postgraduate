...
\begin{document}
    ...
    \begin{enumerate}
        ...
        \begin{itemize}
            ...
            \item $\dint \tan x\,\mathrm{d}x = -\ln |\cos x| + C = \ln |\sec x| + C$
            ...
            \item $\dint \dfrac{1}{\sin x}\,\mathrm{d}x = \dint \csc x\,\mathrm{d}x = \ln |\csc x - \cot x| + C = -\ln |\csc x + \cot x| + C$
        ...
        \begin{itemize}
            ...
            \end{circlenum}
            \item $\dint \sqrt{1+x^2}\,\mathrm{d}x = \dfrac{x\sqrt{1+x^2}}{2} + \dfrac{1}{2}\ln\!\left(x+\sqrt{1+x^2}\right) + C$
        \end{itemize}
        \item 部分分式{\color{red}{分解}}型
        \begin{itemize}
            \item $\dint \dfrac{1}{x^2 - a^2}\,\mathrm{d}x = \dint \dfrac{1}{2a}(\dfrac{1}{x-a} - \dfrac{1}{x+a})\,\mathrm{d}x = \dfrac{1}{2a}(\ln|x - a| - \ln|x + a|) + C = \dfrac{1}{2a}\ln \left|\dfrac{x-a}{x+a}\right| + C$
            \item $\dint \dfrac{1}{a^2 - x^2}\,\mathrm{d}x = \dint \dfrac{1}{2a}(\dfrac{1}{x + a} - \dfrac{1}{x - a})\,\mathrm{d}x = \dfrac{1}{2a}\ln \left|\dfrac{x+a}{x-a}\right| + C$
        \end{itemize}
        \item 三角函数降幂公式型
        \begin{itemize}
        ...
        \item $n$次方
        \begin{itemize}
            \item $\dint \tan^n x\,\mathrm{d}x = \dint \tan^{n-2} x(\sec^2 x - 1)\,\mathrm{d}x = \dint \tan^{n-2} x\,\mathrm{d}(\tan x) - \dint \tan^{n-2} x\,\mathrm{d}x = \dfrac{\tan^{n-1} x}{n - 1} - \dint \tan^{n-2} x\,\mathrm{d}x$
        \end{itemize}
    \end{enumerate}
    ...
    \begin{enumerate}
        ...
        \[
            ...
            = \int f(u)\,\mathrm{d}u.
        \]
        \item $\dfrac{1}{\sqrt{x}(1+x)}\,\mathrm{d}x = \mathrm{d}(\arctan \sqrt{x})$
        ...
        \[
            ...
            = \int f(u)\,\mathrm{d}u.
        \]
        \item $-\dfrac{x}{\sqrt{1 - x^2}}\,\mathrm{d}x = \mathrm{d}(\sqrt{1 - x^2})$
        \item $f'(x)\,\mathrm{d}x = \mathrm{d}[f(x)]$
    ...
    \begin{enumerate}
        ...
        \item 思想：若$Q_m(x)$在实数域内可{\color{red}{因式分解}}，则因式分解后再把$\displaystyle\frac{P_n(x)}{Q_m(x)}$拆成若干项最简有理分式之和。有理式$=$\textbf{最简有理分式之和}，其中最简有理式分为
        \begin{circlenum}
        ...
        \begin{enumerate}
            ...
            \begin{analysisbox}
                ...
                原积分：
                \[
                    I=\int\frac{1}{a+b\cos x}\,\mathrm{d}x
                \]
                ...
                使用：
                \[
                    \int\frac{1}{u^2+\alpha^2}\,\mathrm{d}u
                    =
                    \frac1\alpha\arctan\frac{u}{\alpha}+C
                \]
    ...
    \begin{enumerate}
        ...
        \item 区间再现公式：设$f(x)$为连续函数，则$\dint_{a}^{b} f(x)\,\mathrm{d}x = \dint_{a}^{b} f(a + b - x)\,\mathrm{d}x$。（\textbf{本质：}把区间左右翻过来看。整个区间仍然完整扫了一遍
        \begin{enumerate}
    ...
    \begin{enumerate}
        ...
        \item ${\color{red}{\bigstar}}$\; $f(x)$ 为可积（\textbf{有可能有跳跃间断点、可去间断点}）的奇函数 $\Rightarrow \dint_{\color{red}{a}}^{x} f(t)\,\mathrm{d}t,\ \forall a$ 为偶函数

        \item 若 $f(x)$ 为连续的奇函数，则 $\dint_{\color{red}{a}}^{x} f(t)\,\mathrm{d}t + C$ 也是偶函数，故 $f(x)$ 的全体原函数均为偶函数

        \item ${\color{red}{\bigstar}}$\; $f(x)$ 为可积的偶函数 $\Rightarrow$
        \[
            \begin{cases}
                \dint_{\color{red}{a}}^{x} f(t)\,\mathrm{d}t \text{ 为奇函数}, \\[6pt]
                \dint_{\color{red}{a}}^{x} f(t)\,\mathrm{d}t \ (a \neq 0)
                \begin{cases}
    ...
    \begin{enumerate}
        \item 在计算反常积分（\textbf{定积分}）时，注意识别{\color{red}{奇点}}（端点、{\color{red}{内部}}）(瑕点，$\pm \infty$)
    ...
    \begin{enumerate}
        ...
        因此
        \begin{gather*}
            \Gamma(n+1)= \dint_0^{+\infty} x^{n}e^{-x}\,\mathrm{d}x = n!(n\text{为非负整数}) \\
    ...
    \begin{enumerate}
        ...
        \item 判断$f(x) = \ln (x + \sqrt{x^2 + 1})$奇偶性
        \[
            f(-x) = \ln (\sqrt{x^2 + 1} - x) = \ln \frac{1}{\sqrt{x^2 + 1} + x} = -f(x)
        \]
        \item $\dint_0^{\pi} |\cos \theta|^3\,\mathrm{d}\theta = 2\dint_0^{\pi/2} \cos^3 \theta\,\mathrm{d}\theta$
        \[
            \begin{aligned}
                \int_0^{\pi} |\cos \theta|^3\,\mathrm{d}\theta
                &= \int_0^{\pi/2} \cos^3 \theta\,\mathrm{d}\theta + \int_{\pi/2}^{\pi} (-\cos \theta)^3\,\mathrm{d}\theta \\
                &= \int_0^{\pi/2} \cos^3 \theta\,\mathrm{d}\theta + \int_{\pi/2}^{\pi} (-\cos^3 \theta)\,\mathrm{d}\theta \\
                &= \int_0^{\pi/2} \cos^3 \theta\,\mathrm{d}\theta + \int_0^{\pi/2} \cos^3 u\,\mathrm{d}u \\
                &= 2\int_0^{\pi/2} \cos^3 \theta\,\mathrm{d}\theta.
            \end{aligned}
        \]
        其中在$[0,\pi/2]$上$\cos\theta\ge 0$，在$[\pi/2,\pi]$上$\cos\theta\le 0$，所以可先按区间去绝对值，再用代换$u=\pi-\theta$把后半段化到前半段。

        也可用\textbf{区间再现公式}理解：令$f(\theta) = |\cos\theta|^3$，则
        \[
            f(\pi - \theta) = |\cos(\pi - \theta)|^3 = |-\cos\theta|^3 = |\cos\theta|^3 = f(\theta)
        \]
        因而$f(\theta)$关于$\theta = \dfrac{\pi}{2}$对称，所以
        \[
            \int_0^{\pi} |\cos \theta|^3\,\mathrm{d}\theta
            = 2\int_0^{\pi/2} |\cos \theta|^3\,\mathrm{d}\theta
            = 2\int_0^{\pi/2} \cos^3\theta\,\mathrm{d}\theta.
        \]
    ...
    \begin{enumerate}
        ...
        \item $\dfrac{1}{x^2 + x} = \dfrac{1}{(x+\frac{1}{2})^2 - (\frac{1}{2})^2}$
        \item 遇到$e^{ax}$首先想到分部积分法，将$\dint e^{ax}\,\mathrm{d}x = \dfrac{1}{a} \dint \,\mathrm{d}(e^{ax}) $
        \item 分段函数求原函数的时候要注意{\color{red}{连续性}}，从而确定$C$
    \end{enumerate}
