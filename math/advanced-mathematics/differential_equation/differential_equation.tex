...
% =========================
    ...
    未知函数是一元函数（即只有一个自变量）的微分方程，称为\textbf{常微分方程}（Ordinary Differential Equation, ODE）
    ...
    \begin{itemize}
        ...
        \item \textbf{解法}：令 $u = \dfrac{y}{x}$（即 $y = xu$），则 $\dfrac{\mathrm{d}y}{\mathrm{d}x} = u + x\dfrac{\mathrm{d}u}{\mathrm{d}x}$，代入后得到关于 $u$ 的可分离变量方程：
        \[
    ...
    \begin{itemize}
        ...
        \begin{circlenum}
            ...
            \item 代入得一阶线性方程：$\dfrac{\mathrm{d}z}{\mathrm{d}x} + (1-n)p(x)z = (1-n)q(x)$
        \end{circlenum}
    \end{itemize}
    ...
    \begin{enumerate}
        \item $y'' = f(x, y')$型（方程中不显含 $y$） \textbf{解法}
        \begin{circlenum}
            \item 令 $p = y' = p(x)$，则 $y'' = p' = \dfrac{dp}{dx}$，则原方程变为一阶方程 $\displaystyle\frac{dp}{dx} = f(x, p)$
            \item 若求得其通解为 $p = \varphi(x, C_1)$，即 $y' = \varphi(x, C_1)$，则原方程的通解为 $y = \int \varphi(x, C_1)\mathrm{d}x + C_2$
        \end{circlenum}
        \item $y'' = f(y, y')$型（方程中不显含自变量 $x$） \textbf{解法}
        \begin{circlenum}
            \item 令 $p = y' = p(y)$，则 $y'' = \dfrac{dp}{dx} = \dfrac{dp}{dy} \cdot \dfrac{dy}{dx} = p\dfrac{dp}{dy}$，则原方程变为一阶方程 $\displaystyle p\frac{dp}{dy} = f(y, p)$
            \item 若求得其通解为 $p = \varphi(y, C_1)$，即 $y' = \varphi(y, C_1)$，则分离变量积分得 $\dfrac{dy}{\varphi(y, C_1)} = dx$
            \item 两边积分得 $\displaystyle\int \dfrac{dy}{\varphi(y, C_1)} = x + C_2$，即可求得原方程的通解
        \end{circlenum}
        \item $y'' = f(y')$型 \textbf{解法}
        \begin{circlenum}
            \item 令 $p = y'$，则 $y'' = p' = \dfrac{dp}{dx}$，原方程变为 $\dfrac{dp}{dx} = f(p)$
            \item 分离变量：$\dfrac{dp}{f(p)} = dx$，积分得 $p = \varphi(x, C_1)$，即 $y' = \varphi(x, C_1)$
            \item 再积分得原方程通解：$y = \int \varphi(x, C_1)\mathrm{d}x + C_2$
        \end{circlenum}
    \end{enumerate}
    ...
    \begin{enumerate}
        ...
        \begin{itemize}
            ...
            \begin{enumerate}
                \item 当自由项 $f(x) = P_n(x)e^{\alpha x}$，特解设为 $y^* = e^{\alpha x} Q_n(x)x^k$，其中
                \[
                    \begin{cases}
                ...
                \item 当自由项 $f(x) = e^{\alpha x}[P_m(x)\cos\beta x + P_n(x)\sin\beta x]$，特解设为
                \[
                ...
                \[
                    \begin{cases}
                        ...
                        k = \begin{cases}
                                0, & \alpha + \beta i \text{ 不是特征根，} \alpha + \beta i \neq r_{1, 2} \\
                                1, & \alpha + \beta i \text{ 是特征根，} \alpha + \beta i = r_{1, 2}
                        \end{cases}
                    \end{cases}
                \]
            \end{enumerate}
            ...
            \begin{enumerate}
                ...
                \begin{enumerate}
                    ...
                    \item 记 $F(D) = D^2 + pD + q$，则 $F(D)y = f(x)$，即 $y^* = \dfrac{1}{F(D)}f(x)$
                    \item $D$表示求导的条件下，$\displaystyle\frac{1}{D}$表示积分
                \end{enumerate}
                \item \textbf{算子法常见类型}：
                ...
                \begin{enumerate}
                    ...
                    \begin{circlenum}
                        ...
                        \[
                            y^* = \dfrac{x}{2\beta}\sin\beta x \quad \text{或} \quad y^* = -\dfrac{x}{2\beta}\cos\beta x
                        \]
                    \end{circlenum}
                    ...
                    \]
                    其中 $Q_k(D)$ 是将 $\dfrac{1}{F(D)}$ 展开并截断到 $D^k$ 项，即 $b_0 + b_1D + b_2D^2 + \cdots + b_kD^k$
    ...
    \begin{enumerate}
        ...
        \begin{circlenum}
            ...
            \]
            \[
                \frac{d^2y}{dx^2} = \frac{d}{dx}\left(\frac{1}{x}\frac{dy}{dt}\right) = -\frac{1}{x^2}\frac{dy}{dt} + \frac{1}{x}\frac{d^2y}{dt^2}\frac{dt}{dx} = \frac{1}{x^2}\left(\frac{d^2y}{dt^2} - \frac{dy}{dt}\right)
            \]
            方程化为关于 $t$ 的常系数线性方程
    ...
    \textbf{核心思路}：根据物理定律建立微分方程，常见类型包括：
