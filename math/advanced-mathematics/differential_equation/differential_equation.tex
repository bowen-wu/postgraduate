...
\begin{document}
    ...
    \begin{itemize}
        ...
        \begin{circlenum}
            ...
            \item 代入得{\color{red}{一阶线性微分方程}}：$\dfrac{\mathrm{d}z}{\mathrm{d}x} + (1-n)p(x)z = (1-n)q(x)$
        \end{circlenum}
    \end{itemize}
    ...
    \begin{enumerate}
        ...
        \begin{itemize}
            ...
            \begin{circlenum}
                \item 令 $p = y' = p(x)$，则 $y'' = p' = \dfrac{\mathrm{d}p}{\mathrm{d}x}$，则原方程变为一阶方程 $\displaystyle\frac{\mathrm{d}p}{\mathrm{d}x} = f(x, p)$
        ...
        \begin{itemize}
            ...
            \begin{circlenum}
                \item 令 $p = y' = p(y)$，则 ${\color{red}{y'' = \dfrac{\mathrm{d}p}{\mathrm{d}x}}} = \dfrac{\mathrm{d}p}{\mathrm{d}y} \cdot \dfrac{\mathrm{d}y}{\mathrm{d}x} = p\dfrac{\mathrm{d}p}{\mathrm{d}y}$，则原方程变为一阶方程 $\displaystyle p\frac{\mathrm{d}p}{\mathrm{d}y} = f(y, p)$
                \item 若求得其通解为 $p = \varphi(y, C_1)$，即 $y' = \varphi(y, C_1)$，则{\color{red}{分离变量积分}}得 $\dfrac{\mathrm{d}y}{\varphi(y, C_1)} = \mathrm{d}x$
                \item 两边积分得 $\displaystyle\int \dfrac{\mathrm{d}y}{\varphi(y, C_1)} = x + C_2$，即可求得原方程的通解
            \end{circlenum}
        \end{itemize}
        \item $y'' = f(y')$型
        \begin{itemize}
            ...
            \item \textbf{解法（{\color{red}{赶尽杀绝$y$}}）}
            \begin{circlenum}
                \item 令 $p = y'$，则 $y'' = p' = \dfrac{\mathrm{d}p}{\mathrm{d}x}$，原方程变为 $\dfrac{\mathrm{d}p}{\mathrm{d}x} = f(p)$
                \item 分离变量：$\dfrac{\mathrm{d}p}{f(p)} = \mathrm{d}x$，积分得 $p = \varphi(x, C_1)$，即 $y' = \varphi(x, C_1)$
    ...
    \begin{enumerate}
        ...
        \begin{itemize}
            ...
            \item \textbf{微分算子法}
            \begin{enumerate}
                \item \textbf{基本思想}：引入微分算子 $D = \dfrac{\mathrm{d}}{\mathrm{d}x}$，将微分方程转化为代数运算。
                \item \textbf{算子定义}：
                \begin{enumerate}
                    \item $D = \displaystyle\frac{\mathrm{d}}{\mathrm{d}x}, Dy = y', D^2 = \displaystyle\frac{\mathrm{d}^2}{\mathrm{d}x^2}, D^2y = y''$，$D^ny = y^{(n)}$
        ...
        \begin{itemize}
            ...
            \item \textbf{特解不唯一}：若 $y^*$ 是非齐次方程的特解，$\tilde{y}$ 是对应齐次方程的解，则 $y^* + \tilde{y}$ 仍是该非齐次方程的特解
            \item \textbf{线性无关的判定}：若 $\dfrac{y_1}{y_2} \not\equiv$ 常数，则 $y_1, y_2$ 线性无关；等价地，朗斯基行列式
            \[
                W(y_1, y_2) = \begin{vmatrix}
                                  y_1 & y_2 \\ y_1' & y_2'
                \end{vmatrix} = y_1 y_2' - y_1' y_2 \neq 0
            \]
            \item \textbf{反推方程的思路}（已知解的结构，反求方程）：

            \begin{circlenum}
                \item {\color{red}{剥离齐次解}}：所给特解中，指数（特解中$e$的幂）$\beta \neq \alpha$（自由项中$e$的幂) 的 $e^{\beta x}$ 型项{\color{red}{一定}}不属于标准特解，必为齐次解，其指数 $\beta$ 即为特征根
                \item {\color{red}{判断 $\alpha$ 是否为特征根}}：对所给特解中指数等于 $\alpha$ 的项，检查是否含 $x^k$ 因子。含则{\color{red}{一定是}}特征根（$k$ 重）；不含则{\color{red}{不一定}}——可能是标准特解（$\alpha$ 非特征根），也可能是混入的齐次解（$\alpha$ 是特征根），需进入下一步假设判定
                \item {\color{red}{假设判定}}（仅在不含 $x^k$ 时）：分别假设 $\alpha$ 是/不是特征根，结合第一步已确定的特征根，各自推出特征方程，将特解代入验证，与已知条件矛盾则排除该假设
                \item 由最终确定的全部特征根写出特征方程 $\Rightarrow$ 确定 $p, q$
                \item 剥离齐次解后剩余部分即为标准{\color{red}{特解}}，代入方程定出自由项 $f(x)$
            \end{circlenum}
            \begin{analysisbox}[Example]
                设二阶常系数线性微分方程$y'' + \alpha y' + \beta y = \gamma e^x$的一个特解为$y^* = e^{2x} + (1+x)e^x$，确定常数$\alpha, \beta, \gamma$

                \begin{circlenum}
                    \item {\color{red}{剥离齐次解}}：查看自由项 $\Rightarrow \alpha = 1 \Rightarrow e^{2x}$必定是齐次解，特征根$r_1 = 2$
                    \item {\color{red}{判断 $\alpha$ 是否为特征根}}：特解中指数等于 $\alpha = 1$ 的项 $xe^x$，含 $x^k(k = 1)$ 因子。则$r_2 = 1$是特征根（$1$ 重）
                    \item {\color{red}{假设判定}}（仅在不含 $x^k$ 时）：无
                    \item 特征方程：$(r-1)(r-2) = 0 \Rightarrow \alpha = -3, \beta = 2$
                    \item 剥离齐次解后剩余部分即为标准{\color{red}{特解}}，此时标准特解为$e^x$，将$e^x$带入方程中$\gamma = -1$
                \end{circlenum}
            \end{analysisbox}
        \end{itemize}
    ...
    \begin{enumerate}
        ...
        \item 反求方程的理论基础
        \begin{circlenum}
            \item 如果解中含{\color{red}{特解}} $\bm{e^{rx}}$，则 $r$ 至少为单实根；如二重根 $(C_1 + C_2x)e^{rx}$，令 $C_1 = 1$，$C_2 = 0$
            \item 如果解中含{\color{red}{特解}} $\bm{x^{k-1}e^{rx}}$，则 $r$ 至少为 $k$ 重实根
            \item 如果解中含{\color{red}{特解}} $\bm{e^{\alpha x}\cos\beta x}$ 或 $\bm{e^{\alpha x}\sin\beta x}$，则 $\alpha \pm \beta i$ 至少为单复根
            \item 如果解中含{\color{red}{特解}} $\bm{e^{\alpha x}x\cos\beta x}$ 或 $\bm{e^{\alpha x}x\sin\beta x}$，则 $\alpha \pm \beta i$ 至少为二重复根
        \end{circlenum}
    \end{enumerate}
    ...
    \begin{itemize}
        ...
        \item \textbf{解法}
        \begin{circlenum}
            \item 当 $x > 0$ 时，令 $x = e^t$，则 $t = \ln x$，$\dfrac{\mathrm{d}t}{\mathrm{d}x} = \dfrac{1}{x}$，于是
            \[
            ...
            \]
            \[
                \frac{\mathrm{d}^2y}{\mathrm{d}x^2} = \frac{\mathrm{d}}{\mathrm{d}x}\left(\frac{1}{x}\frac{\mathrm{d}y}{\mathrm{d}t}\right) = -\frac{1}{x^2}\frac{\mathrm{d}y}{\mathrm{d}t} + \frac{1}{x}{\color{red}{\frac{\mathrm{d}^2y}{\mathrm{d}t^2}\frac{\mathrm{d}t}{\mathrm{d}x}}} = \frac{1}{x^2}\left(\frac{\mathrm{d}^2y}{\mathrm{d}t^2} - \frac{\mathrm{d}y}{\mathrm{d}t}\right)
            \]
            方程化为关于 $t$ 的常系数线性方程
    ...
    \begin{enumerate}
        ...
        \begin{itemize}
            ...
            \begin{circlenum}
                ...
                \item 追踪者的速度方向指向目标：$\dfrac{\mathrm{d}y}{\mathrm{d}x} = \dfrac{Y - y}{X - x}$
                \item 结合速度关系建立微分方程
            \end{circlenum}
        \end{itemize}
        \item 解题步骤
    ...
    \textbf{核心思路}：根据物理定律建立微分方程。{\color{red}{注意变化率的正负}}

    \subsection{结论}

    \begin{enumerate}
        \item 设函数 $f(x)$ 在 $x=x_0$ 处二阶可导，且 $f'(x_0)=0$，若 $f''(x_0)<0$，则 $f(x)$ 在 $x=x_0$ 处取得极大值
        \item 当$x \to 0$时，$x \sim \ln(1+x)$
        \item 凑微分的时候，常数直接加上。\emph{e.g.} $\int x\ln(1+x^2)\,\mathrm{d}x = \dfrac{1}{2}\int \ln(1+x^2)\,\mathrm{d}(x^2{\color{red}{ + 1}})$
        \item 为方便合并，不定积分后可加$\ln C$
        \item 通解中的常数是指{\blueuline{在一定范围内任意取值的常数}}，而未必是在实数范围内任意取值的常数
        \item 初始条件不仅可以求独立常数$C$，还可以确定解的符号
        \item 对于一个不定积分求极限，需要将不定积分转化为变限积分，之后查看是否能用洛必达法则
        \item 对于无法直接计算的微分方程$y' = f(x, y)$，重点在于通过什么形式的{\color{red}{换元（简单化）}}将其化成常见的形式
        \item 分情况讨论时，要注意
        \begin{circlenum}
            \item 如何分
            \item 场景全
        \end{circlenum}
        \item 追击点到被追击点的连线始终为该点的切线方向
    \end{enumerate}

    \subsection{理解}

    \begin{enumerate}
        \item 设二阶常系数线性微分方程 $y'' + \alpha y' + \beta y = \gamma e^x$ 的一个特解为 $y^* = e^{2x} + (1+x)e^x$，确定常数 $\alpha, \beta, \gamma$，并求该方程的通解
        \begin{analysisbox}[解]
            由 $y^* = e^{2x} + (1+x)e^x$，得
            \[
            ...
            整理得
            \[
                \underbrace{(4 + 2\alpha + \beta)}_{} e^{2x} + \underbrace{(3 + 2\alpha + \beta)}^{\text{常数项}} e^x + \underbrace{(1 + \alpha + \beta)}_{\text{系数}} x e^x = \gamma e^x
            \]
            比较同类项系数：
            \[
                \begin{cases}
                    4 + 2\alpha + \beta = 0 \quad (e^{2x}\text{ 的系数}) \\
                    1 + \alpha + \beta = 0 \quad (xe^x\text{ 的系数}) \\
                    3 + 2\alpha + \beta = \gamma \quad (e^x\text{ 的系数})
                \end{cases}
            \]
            由前两式解得 $\alpha = -3$，$\beta = 2$，代入第三式得 $\gamma = -1$。

            特征方程 $r^2 - 3r + 2 = 0$ 的根为 $r_1 = 1$，$r_2 = 2$，齐次方程通解 $Y = C_1 e^x + C_2 e^{2x}$，取特解 $y^* = xe^x$，故通解为
            \[
