...
\begin{document}
    ...
    \begin{circlenum}
        \item \emph{e.g.} $f(x) = e^x, f''(x) = e^x > 0$，看图像\textbf{凹的}
    ...
    \begin{enumerate}
        ...
        \item {\color{red}{二阶导数不存在的点也有可能是拐点}}: $f''(x_0)$ 不存在。如 $y=\sqrt[3]{x}$ 在 $(0,0)$ 处的情形
    \end{enumerate}
    ...
    \begin{circlenum}
        ...
        \item 设多项式函数$f(x) = (x-a)^{n}g(x)(n>1)$，且$g(a) \neq 0$，则当$n$为{\color{red}{偶数}}时，$x=a$是$f(x)$的{\color{red}{极值点}}；当$n$为{\color{red}{奇数}}时，点$(a, 0)$是$f(x)$的{\color{red}{拐点}}
    ...
    若$\lim\limits_{{\color{red}{x\to x_0^{+}}}} f(x) = \infty$（或$\lim\limits_{{\color{red}{x\to x_0^{-}}}} f(x) = \infty$），则$x=x_0$为一条铅直渐近线。$x_0$的可能情况为：
    \begin{enumerate}
        \item 函数的无定义点。
    ...
    \begin{enumerate}
        \item 若 $\lim\limits_{\color{red}{x\to +\infty}} f(x)=y_1$，则 $y=y_1$ 为函数 $y=f(x)$ 的一条水平渐近线；
        \item 若 $\lim\limits_{\color{red}{x\to -\infty}} f(x)=y_2$，则 $y=y_2$ 为函数 $y=f(x)$ 的一条水平渐近线；
        \item 若 $\lim\limits_{\color{red}{x\to +\infty}} f(x) = \lim\limits_{{\color{red}{x\to -\infty}}} f(x) = y_0$，则 $y=y_0$ 为函数 $y=f(x)$ 的一条水平渐近线
    \end{enumerate}
    ...
    \begin{enumerate}
        ...
        \item $\color{red}{\bigstar}$ 曲线与渐近线可能会有交点。\emph{e.g.} $y = \displaystyle\frac{\sin x}{x}$
        \begin{center}
            \begin{tikzpicture}
                \begin{axis}
                    [
                    ...
                    domain = -80:80,
                    samples = 800,
                    xmin = -85, xmax = 84,      % 右侧留白
                    ymin = -0.3, ymax = 1.1,    % 上方留白
                    width = 17cm,
                    height = 5.5cm,
                    xtick = \empty,
                    ytick = \empty,
    ...
    \begin{circlenum}
        \item 求出函数 $f(x)$ 在 $(a,b)$ 内的可疑点——驻点与不可导点，并求出这些可疑点处的函数值；
        \item 求区间 $(a,b)$ 两端的单侧极限若 $a,b$ 为有限常数，则求 $\lim\limits_{x\to a^+} f(x)$ 与 $\lim\limits_{x\to b^-} f(x)$；若 $a=-\infty$，则求 $\lim\limits_{x\to -\infty} f(x)$；若 $b=+\infty$，则求 $\lim\limits_{x\to +\infty} f(x)$，记以上所求左端极限为 $A$，右端极限为 $B$；
...
% 最高点
        ...
        参数范围: $0 \leq t \leq 2\pi$

        尖点角度:
        \begin{itemize}
            \item $t = 0 \Rightarrow (a, 0)$
            \item $t = \displaystyle\frac{\pi}{2} \Rightarrow (0, a)$
            \item $t = \pi \Rightarrow (-a, 0)$
            \item $t = \displaystyle\frac{3\pi}{2} \Rightarrow (0, -a)$
        \end{itemize}
...
% x轴交点 (a,0)
    ...
    \begin{enumerate}
        ...
        \item 设$f(x)$在$[a, b]$上可导，则$f(x)$在$[a, b]$上必存在最大（小）值，且
        \begin{itemize}
            \item 若$f(x)$在$x = a$处取$[a, b]$上的最大（小）值，则$f'_+(a) \leq 0(\geq 0)$
            \item 若$f(x)$在$x = b$处取$[a, b]$上的最大（小）值，则$f'_-(b) \geq 0(\leq 0)$
            \item 若$F(x) = \int_a^x f(t)\,\mathrm{d}t, x \in [a, b]$
            \begin{circlenum}
                \item 若$f(x)$在$[a, b]$上可导，且当$f(x)$在$x = a$处取得最大（小）值，有$F''_+(a) \leq 0(\geq 0)$
                \item 若$f(x)$在$[a, b]$上可导，且当$f(x)$在$x = b$处取得最大（小）值，有$F''_-(b) \geq 0(\leq 0)$
                \item 若$f(x)$在$[a, b]$上可导，且当$f(x)$在$x = c \in (a, b)$处取得最大值或最小值是，有$F''(c) = 0$
            \end{circlenum}
        \end{itemize}
    \end{enumerate}
