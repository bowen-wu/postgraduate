...
\begin{document}
    \section{一元函数微分学的应用 - 中值定理、微分等式与微分不等式}

    \subsection{中值定理}

    \subsubsection{涉及函数的中值定理}
    ...
    \begin{enumerate}
        ...
        \begin{itemize}
            ...
            \begin{circlenum}
                ...
                \item $u''v + 2u'v' + uv'' = (uv)''$
            \end{circlenum}
            \item 商的求导公式$\displaystyle\frac{u'v - uv'}{v^2} = (\displaystyle\frac{u}{v})'$
            \begin{circlenum}
                \item $\displaystyle\frac{xf'(x) - f(x)}{x^2} = [\displaystyle\frac{f(x)}{x}]'$ \\ $\Rightarrow$见到$xf'(x) - f(x), x \neq 0$，令$F(x) = \displaystyle\frac{f(x)}{x}$
    ...
    \begin{enumerate}
        ...
        \begin{enumerate}
            ...
            \begin{circlenum}
                ...
                \item 余弦函数$== \sin' x$
                \[
                    \cos x = \sin' x = \sum_{n=0}^{\infty} (-1)^n \frac{x^{2n}}{(2n)!} = 1 - \frac{x^2}{2!} + \frac{x^4}{4!} - \cdots, \quad -\infty < x < +\infty \\
                \]
    ...
    \begin{enumerate}
        ...
        \[
            ...
            \end{aligned}
        \]
        \item 当$x \to +\infty$时，$f'(x) \to 0$，意味着当$x \to +\infty$时，$f(x)$的变化率$\to 0$，于是$f(x+1) - f(x) \to 0(x\to +\infty)$
    \end{enumerate}

    \subsection{微分等式}

    方程$f(x) = 0$的根就是函数$f(x)$的零点。从几何上讲，方程的根作为两条曲线的交点。基于此，讨论方程的根 $==$ 讨论曲线的交点。方法步骤如下
    ...
    \subsubsection{用函数性态（包括单调性、凹凸性和最值等）证明不等式}
    ...
    \begin{enumerate}
        \item 当$0 < x < \displaystyle\frac{\pi}{2}$时，$\displaystyle\frac{2x}{\pi} < \sin x < x < \tan x$
    \end{enumerate}
